...
describe('Testing uiReducer', () => {
    it('verify the state returned by the uiReducer funcion equals the initial state when no action is passed', () => {
        const res = uiReducer();
        expect(res.toJS()).toStrictEqual(initialState.toJS())
    });

    it('verify the state returned by the uiReducer function equals the initial state when the SELECT_COURSE is passed', () => {
